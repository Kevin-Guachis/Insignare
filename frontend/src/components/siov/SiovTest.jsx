import { siovWarning } from "./alerts";
...
export default function SiovTest({ questions,answers,setAnswers,onFinish }){
 ...
 function finish(){
  ...
  if(missing.length){siovWarning("Preguntas pendientes","Faltan por responder las preguntas: "+missing.join(", "));go(missing[0]);return;}
  onFinish();
 }
 ...
 {current===questions.length?<button className="btn btn-brand" onClick={finish}>Ver Resultados →</button>:<button className="btn btn-brand" onClick={()=>{if(answers[current]===undefined){siovWarning("Pregunta pendiente","Debes responder esta pregunta antes de continuar.");return;}go(current+1);}}>Siguiente →</button>}
 </div></section>;
}
