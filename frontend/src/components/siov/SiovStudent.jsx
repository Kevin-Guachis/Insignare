...
export default function SiovStudent({ onBack,onStart,initialStudent }){
 ...
 <p className="small text-muted">Estos datos se usan solo en memoria para tu informe. No se envían ni se guardan en el servidor.</p>
 <form id="form-datos" className="row g-3 mt-2" noValidate onSubmit={e=>{e.preventDefault();onStart(student);}}>
 {fields.map(([key,label,type])=><div className="col-md-6" key={key}><label className="form-label" htmlFor={"inp-"+key}>{label} *</label><input autoComplete="off" id={"inp-"+key} className="form-control" type={type} required min={type==="number"?10:undefined} max={type==="number"?99:undefined} value={student[key]||""} onChange={e=>setStudent({...student,[key]:e.target.value})}/></div>)}
