...
export default function SiovResults({ results,content,student,onReset }){
 ...
 const [p,s,t]=results;
 const nombre=(student.nombres||"El/La estudiante").split(" ")[0];
 ...
 <SiovCharts results={results} barRef={barRef} radarRef={radarRef} onReady={markReady} onError={setError}/>
 ...
 <button className="btn-download-primary download-pulse" disabled={!ready||busy} onClick={async()=>{setBusy(true);setError("");try{await generarInformePDF({state:{estudiante:student,comparador:comparison},resultadoAreas:results,...content,barCanvas:barRef.current,radarCanvas:radarRef.current});}catch(e){setError(e.message||"No se pudo generar el PDF.");}finally{setBusy(false);}}}>{busy?"Generando informe...":"⬇️ Descargar Informe PDF"}</button>
