...
export default function ExamAdmin({ university, onClose }) {
 ...
 useEffect(()=>{
  let active=true;
  getAdminExam(university.id).then(data=>{if(active){setExam(data.exam||{...blank});setCategories(data.categories);setError("");}}).catch(e=>{if(active){setError("No se pudo cargar la información.");showError(e.message);}}).finally(()=>{if(active)setLoading(false);});
  return()=>{active=false;};
 },[university.id,retry]);
 async function handleSaveExam(e){
  e.preventDefault();setBusy(true);setError("");
  try{setExam(await saveExam({...exam,university_id:university.id}));showSuccess("Examen guardado correctamente.");}
 ...
 return <section className="admin-news__card exam-admin" aria-labelledby="exam-admin-title">
  ...
  <form className="admin-news-form" onSubmit={handleSaveExam}><fieldset disabled={busy||!!editor}>
   ...
   <div className="admin-news-actions"><button type="submit" className="admin-news-primary">{busy?"Guardando...":"Guardar examen"}</button>
    {exam.id&&<button type="button" className="admin-news-secondary admin-icon-button" title="Eliminar examen" aria-label="Eliminar examen" onClick={()=>remove({kind:"exam"})}><i className="bi bi-trash" aria-hidden="true"/></button>}
   </div>
  </fieldset></form>
