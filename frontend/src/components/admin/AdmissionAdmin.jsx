...
export default function AdmissionAdmin({ university, onClose }) {
 ...
 useEffect(()=>{
  let active=true;
  getAdminAdmission(university.id).then(data=>{if(active){setProcess(data.admission||{...blank});setSteps(data.steps);setError("");}}).catch(e=>{if(active){setError("No se pudo cargar la información.");showError(e.message);}}).finally(()=>{if(active)setLoading(false);});
  return()=>{active=false;};
 },[university.id,retry]);
 async function saveProcess(e){
  e.preventDefault();setBusy(true);setError("");
  try{setProcess(await saveAdmission({...process,university_id:university.id}));showSuccess("Proceso guardado correctamente.");}
 ...
 return <section className="admin-news__card admission-admin" aria-labelledby="admission-admin-title">
  ...
  <form className="admin-news-form" onSubmit={saveProcess}><fieldset disabled={busy||!!editor}>
   ...
   <div className="admin-news-actions"><button type="submit" className="admin-news-primary">{busy?"Guardando...":"Guardar proceso"}</button>
    {process.id&&<button type="button" className="admin-news-secondary admin-icon-button" title="Eliminar proceso" aria-label="Eliminar proceso" onClick={()=>remove({kind:"process"})}><i className="bi bi-trash" aria-hidden="true"/></button>}
   </div>
  </fieldset></form>
