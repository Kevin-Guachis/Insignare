...
export default function UniversityDocumentsAdmin({ university,onClose }){
 ...
 useEffect(()=>{
  let active=true;
  listAdminUniversityDocuments(university.id).then(data=>{if(active){setRows(data);setError("");}}).catch(e=>{if(active){setError("No se pudo cargar la información.");showError(e.message);}}).finally(()=>{if(active)setLoading(false);});
  return()=>{active=false;};
 },[university.id,retry]);
 ...
 async function remove(target) {
  if (busy || !await confirmAction("¿Eliminar documento?", `El documento “${target.titulo}” se eliminará definitivamente.\n\nEsta acción no se puede deshacer.`, "Eliminar")) return;
  setBusy(true);setError("");
  try{await deleteUniversityDocument(target.id,university.id);setRows(current=>current.filter(row=>row.id!==target.id));showSuccess("Documento eliminado.");}
  catch(e){showError(e.message);}finally{setBusy(false);}
 }
 return <section className="admin-news__card" aria-labelledby="university-documents-admin-title">
  ...
  {editor?<UniversityDocumentForm key={editor.id||"new"} document={editor} universityId={university.id} onCancel={()=>setEditor(null)} onSaved={row=>{setRows(current=>sorted([...current.filter(item=>item.id!==row.id),row]));setEditor(null);showSuccess("Documento guardado correctamente.");}}/>:<>
   <div className="admin-news__toolbar"><button type="button" className="admin-news-primary" disabled={busy||loading} onClick={()=>{setEditor({...blank});}}>Nuevo documento</button></div>
   {loading?<p role="status">Cargando documentos...</p>:<div className="admin-news-table-wrapper"><table className="admin-news-table"><thead><tr><th>Documento</th><th>Estado</th><th>Orden</th><th>Acciones</th></tr></thead><tbody>
    {rows.map(row=><tr key={row.id}><td data-label="Documento"><p>{row.titulo}</p><small>{row.documento_nombre}</small></td><td data-label="Estado"><span className={row.activo ? "admin-status admin-status--active" : "admin-status admin-status--inactive"}>{row.activo ? "Activo" : "Inactivo"}</span></td><td data-label="Orden">{row.orden}</td><td data-label="Acciones"><div className="admin-news-actions">
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>{setEditor({...row});}} title="Editar documento" aria-label="Editar documento"><i className="bi bi-pencil" aria-hidden="true"/></button>
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>toggle(row)} title={row.activo?"Desactivar documento":"Activar documento"} aria-label={row.activo?"Desactivar documento":"Activar documento"}><i className={row.activo?"bi bi-eye":"bi bi-eye-slash"} aria-hidden="true"/></button>
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>remove(row)} title="Eliminar documento" aria-label="Eliminar documento"><i className="bi bi-trash" aria-hidden="true"/></button>
    </div></td></tr>)}
