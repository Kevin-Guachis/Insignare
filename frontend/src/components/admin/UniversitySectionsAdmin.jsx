...
export default function UniversitySectionsAdmin({ university }) {
 ...
 useEffect(()=>{
  let active=true;
  listAdminUniversitySections(university.id).then(data=>{if(active){setRows(data);setError("");}}).catch(e=>{if(active){setError("No se pudo cargar la información.");showError(e.message);}}).finally(()=>{if(active)setLoading(false);});
  return()=>{active=false;};
 },[university.id,retry]);
 async function save(values,editing=false) {
  ...
  try {
   ...
   if(editing)setForm(null);showSuccess("Sección guardada correctamente.");
  }catch(e){showError(e.message);}finally{setBusy(false);}
 }
 async function remove(target) {
  if (busy || !await confirmAction("¿Eliminar sección?", `La sección “${target.titulo}” se eliminará definitivamente.\n\nEsta acción no se puede deshacer.`, "Eliminar")) return;
  setBusy(true);setError("");
  try{await deleteUniversitySection(target.id,university.id);setRows(current=>current.filter(row=>row.id!==target.id));showSuccess("Sección eliminada.");}
  catch(e){showError(e.message);}finally{setBusy(false);}
 }
 ...
 return <section className="admin-news__card" aria-labelledby="university-sections-admin-title">
  <div className="admin-news__toolbar"><div><h2 id="university-sections-admin-title">Secciones</h2><p>Universidad: {university.nombre}</p></div>
   <button type="button" className="admin-news-primary" disabled={busy||!!form} onClick={()=>{setForm({tipo:"admission",titulo:"Proceso de admisión",orden:0,activo:1});}}>Nueva sección</button>
  </div>
  ...
  {form?<form className="admin-news-form" onSubmit={e=>{e.preventDefault();save(form,true);}}>
   ...
   {loading?<p role="status">Cargando secciones...</p>:<div className="admin-news-table-wrapper"><table className="admin-news-table"><thead><tr><th>Título</th><th>Tipo</th><th>Orden</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>
    {rows.map(row=><tr key={row.id}><td data-label="Título">{row.titulo}</td><td data-label="Tipo">{universitySectionTypes[row.tipo]||row.tipo}</td><td data-label="Orden">{row.orden}</td><td data-label="Estado"><span className={row.activo ? "admin-status admin-status--active" : "admin-status admin-status--inactive"}>{row.activo ? "Activo" : "Inactivo"}</span></td><td data-label="Acciones"><div className="admin-news-actions">
     ...
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>save({...row,activo:row.activo?0:1})} title={row.activo?"Desactivar sección":"Activar sección"} aria-label={row.activo?"Desactivar sección":"Activar sección"}><i className={row.activo?"bi bi-eye":"bi bi-eye-slash"} aria-hidden="true"/></button>
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>remove(row)} title="Eliminar sección" aria-label="Eliminar sección"><i className="bi bi-trash" aria-hidden="true"/></button>
    </div></td></tr>)}
