...
import { siovConfirm,siovError } from "../siov/alerts";
...
export default function SiovAdmin(){
 ...
 useEffect(()=>{let active=true;getSiovAdmin().then(d=>{if(active){setData(d);setError("");}}).catch(e=>{if(active)setError(e.message);siovError(e.message);});return()=>{active=false;};},[retry]);
 async function save(row){setBusy(true);setError("");try{await saveSiov(type,row);setData(await getSiovAdmin());setEditor(null);setMessage("Contenido guardado. Los tests en curso conservan su versión inicial.");}catch(e){setError(e.message);siovError(e.message);}finally{setBusy(false);}}
 async function remove(row){if(!await siovConfirm("¿Eliminar contenido?","Esta acción eliminará el contenido seleccionado del SIOV.","Sí, eliminar"))return;setBusy(true);setError("");try{await deleteSiov(type,row.id);setData(await getSiovAdmin());setMessage("Contenido eliminado.");}catch(e){setError(e.message);siovError(e.message);}finally{setBusy(false);}}
