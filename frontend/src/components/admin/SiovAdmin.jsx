...
export default function SiovAdmin(){
 const [data,setData]=useState(null),[type,setType]=useState("questions"),[editor,setEditor]=useState(null),[busy,setBusy]=useState(false),[error,setError]=useState(""),[retry,setRetry]=useState(0);
 useEffect(()=>{let active=true;getSiovAdmin().then(d=>{if(active){setData(d);setError("");}}).catch(e=>{if(active){setError("No se pudo cargar la información.");showError(e.message);}});return()=>{active=false;};},[retry]);
 async function save(row){setBusy(true);setError("");try{await saveSiov(type,row);setData(await getSiovAdmin());setEditor(null);showSuccess("Contenido guardado", "Los tests en curso conservan su versión inicial.");}catch(e){showError(e.message);}finally{setBusy(false);}}
 async function remove(row){if(!await confirmAction("¿Eliminar contenido?","Esta acción eliminará el contenido seleccionado del SIOV. No se puede deshacer.","Eliminar"))return;setBusy(true);setError("");try{await deleteSiov(type,row.id);setData(await getSiovAdmin());showSuccess("Contenido eliminado.");}catch(e){showError(e.message);}finally{setBusy(false);}}
 ...
 return <section className="admin-news__card siov-admin" aria-labelledby="siov-admin-title"><div className="admin-news__toolbar"><h2 id="siov-admin-title">Administración SIOV</h2><button className="admin-news-secondary" disabled={busy} onClick={()=>setRetry(n=>n+1)}>Actualizar datos</button></div>
 ...
 <nav className="siov-admin-tabs" aria-label="Contenido SIOV">{Object.entries(definitions).map(([key,d])=><button key={key} className={type===key?"admin-news-primary":"admin-news-secondary"} disabled={busy} onClick={()=>{setType(key);setEditor(null);}}>{d.label}</button>)}<button className={type==="metrics"?"admin-news-primary":"admin-news-secondary"} onClick={()=>{setType("metrics");setEditor(null);}}>Métricas</button></nav>
 {error&&<p className="admin-login__error" role="alert">{error}</p>}
 {!data?<p role="status">{error ? "Usa Actualizar datos para reintentar." : "Cargando contenido..."}</p>:type==="metrics"?<>
 <h3>Métricas agregadas</h3><dl className="siov-metric-cards"><div><dt>Tests iniciados</dt><dd>{data.metrics.find(m=>m.evento==="started")?.cantidad||0}</dd></div><div><dt>Tests completados</dt><dd>{completed?.cantidad||0}</dd></div><div><dt>Tiempo medio</dt><dd>{completed?.cantidad?(completed.minutos_totales/completed.cantidad).toFixed(1):"0"} min</dd></div></dl><p className="siov-metric-privacy">No se almacenan resultados individuales, respuestas ni datos personales.</p><h4>Detalle de métricas</h4>
 <div className="admin-news-table-wrapper"><table className="admin-news-table"><thead><tr><th>Métrica</th><th>Contenido</th><th>Total</th></tr></thead><tbody>{data.metrics.map(m=><tr key={m.evento+":"+m.dimension_id}><td data-label="Métrica">{{started:"Tests iniciados",completed:"Tests completados",orientation_requested:"Solicitudes de asesoría",area:"Área predominante",career:"Carrera comparada",university:"Consulta de universidad"}[m.evento]}</td><td data-label="Contenido">{data[{area:"areas",career:"careers",university:"universities"}[m.evento]]?.find(r=>r.id===m.dimension_id)?.nombre||"General"}</td><td data-label="Total">{m.cantidad}</td></tr>)}</tbody></table></div>
 ...
 <button className="admin-news-secondary admin-icon-button" disabled={busy} title="Editar" aria-label="Editar" onClick={()=>setEditor({...row})}><i className="bi bi-pencil" aria-hidden="true"/></button>
 <button className="admin-news-secondary admin-icon-button" disabled={busy} title={row.activo?"Desactivar":"Activar"} aria-label={row.activo?"Desactivar":"Activar"} onClick={()=>save({...row,activo:row.activo?0:1})}><i className={row.activo?"bi bi-eye":"bi bi-eye-slash"} aria-hidden="true"/></button>
 <button className="admin-news-secondary admin-icon-button" disabled={busy} title="Eliminar" aria-label="Eliminar" onClick={()=>remove(row)}><i className="bi bi-trash" aria-hidden="true"/></button>
 </div></td></tr>)}
 </tbody></table></div>
 </>}
 </section>;
