...
import { siovError, siovWarning, siovSuccess, siovConfirm } from "../siov/alerts";
...
export default function MaterialResourcesAdmin() {
 ...
 async function save(event){
  ...
  if(!editor.title.trim()||!editor.description.trim()||!editor.university_id||(!file&&!editor.file_path)){
   await siovWarning("Datos incompletos","Selecciona la universidad, título, descripción y un PDF.");return;
  }
  setBusy(true);
  try{
   ...
   await siovSuccess("Recurso guardado","El material se actualizó correctamente.");
  }catch(e){await siovError(e.message);}finally{setBusy(false);}
 }
 ...
 async function remove(row){
  if(!await siovConfirm("¿Eliminar recurso?","Esta acción eliminará el recurso definitivamente.","Eliminar"))return;
  setBusy(true);try{await deleteMaterialResource(row.id);refresh();}catch(e){await siovError(e.message);}finally{setBusy(false);}
 }
 ...
 return <section className="admin-news__card material-admin">
  ...
  {editor?<form className="admin-news-form" onSubmit={save}><fieldset disabled={busy}>
   ...
   <label className="material-field">Archivo PDF *<input type="file" accept=".pdf,application/pdf" required={!editor.file_path} onChange={e=>{
    ...
    if(selected&&(!/\.pdf$/i.test(selected.name)||selected.size>10*1024*1024)){e.target.value="";setFile(null);siovWarning("Archivo inválido","Selecciona un PDF de hasta 10 MB.");return;}
   ...
   {loading?<p role="status">Cargando recursos...</p>:<div className="admin-news-table-wrapper"><table className="admin-news-table"><thead><tr><th>Recurso</th><th>Universidad</th><th>Tipo / Año</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>
    {filterMaterials(rows,filters).map(row=><tr key={row.id}><td data-label="Recurso"><strong>{row.title}</strong><p>{row.subject}</p><small>Orden: {row.display_order}</small></td><td data-label="Universidad">{row.university_name}</td><td data-label="Tipo / Año">{row.type==="prueba"?"Prueba":"Material gratuito"} / {row.year}</td><td data-label="Estado">{row.visible?"Visible":"Oculto"}</td><td data-label="Acciones"><div className="admin-news-actions">
     <button className="admin-news-secondary" disabled={locked} title="Editar recurso" aria-label="Editar recurso" onClick={()=>edit(row)}><i className="bi bi-pencil" aria-hidden="true"/></button>
     <button className="admin-news-secondary" disabled={locked} title={row.visible?"Ocultar recurso":"Mostrar recurso"} aria-label={row.visible?"Ocultar recurso":"Mostrar recurso"} onClick={()=>toggle(row)}><i className={row.visible?"bi bi-eye":"bi bi-eye-slash"} aria-hidden="true"/></button>
     <button className="admin-news-secondary" disabled={locked} title="Eliminar recurso" aria-label="Eliminar recurso" onClick={()=>remove(row)}><i className="bi bi-trash" aria-hidden="true"/></button>
    </div></td></tr>)}
