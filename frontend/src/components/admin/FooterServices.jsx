...
export default function FooterServices() {
 ...
 useEffect(()=>{
  let active=true;
  listAdminFooterServices().then(data=>{if(active)setRows(data);}).catch(e=>{if(active){setError("No se pudo cargar la información.");showError(e.message);}}).finally(()=>{if(active)setLoading(false);});
  return()=>{active=false;};
 },[]);
 ...
 async function remove(target) {
  if (busy || !await confirmAction("¿Eliminar servicio?", `El servicio “${target.nombre}” se eliminará definitivamente.\n\nEsta acción no se puede deshacer.`, "Eliminar")) return;
  setBusy(true);setError("");
  try {await deleteFooterService(target.id);setRows(current=>current.filter(row=>row.id!==target.id));showSuccess("Servicio eliminado.");}
 ...
 return <section className="admin-news__card" aria-labelledby="footer-services-title-admin">
  ...
  {form ? <form className="admin-news-form" onSubmit={event=>{event.preventDefault();save(form,true);}}>
   ...
   {loading?<p role="status">Cargando servicios...</p>:<div className="admin-news-table-wrapper"><table className="admin-news-table"><thead><tr><th>Nombre</th><th>Enlace</th><th>Orden</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>
    {rows.map(row=><tr key={row.id}><td data-label="Nombre">{row.nombre}</td><td data-label="Enlace">{row.enlace||"Sin enlace"}</td><td data-label="Orden">{row.orden}</td><td data-label="Estado"><span className={row.activo ? "admin-status admin-status--active" : "admin-status admin-status--inactive"}>{row.activo ? "Activo" : "Inactivo"}</span></td><td data-label="Acciones"><div className="admin-news-actions">
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>{setForm({...row});}} title="Editar servicio" aria-label="Editar servicio"><i className="bi bi-pencil" aria-hidden="true" /></button>
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>save({...row,activo:row.activo?0:1})} title={row.activo?"Desactivar servicio":"Activar servicio"} aria-label={row.activo?"Desactivar servicio":"Activar servicio"}><i className={row.activo?"bi bi-eye":"bi bi-eye-slash"} aria-hidden="true" /></button>
     <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={()=>remove(row)} title="Eliminar servicio" aria-label="Eliminar servicio"><i className="bi bi-trash" aria-hidden="true" /></button>
    </div></td></tr>)}
