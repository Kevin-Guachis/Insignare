...
import { siovConfirm as confirmAction, siovError as showError, siovWarning as warning, siovSuccess as success } from "../siov/alerts";
...
export default function TestimonialsAdmin() {
 ...
 useEffect(() => {
  ...
  }).catch(e => { if (active) { setError(e.message); showError(e.message); } })
 ...
 async function submit(event) {
  ...
  if (invalid) { await warning("Datos incompletos", invalid); return; }
  if (kind === "testimonials" && (!Number.isInteger(editor.orden) || editor.orden < 0 || editor.orden > 2147483647)) { await warning("Orden inválido", "Escribe un número entero mayor o igual a cero."); return; }
  if (kind === "testimonials" && !file && !editor.imagen) { await warning("Fotografía pendiente", "Selecciona una fotografía para el testimonio."); return; }
  setBusy(true);
  try {
   const row = { ...editor };
   if (file) row.imagen = (await uploadTestimonialPhoto(file)).imagen;
   await persist(row); await success("Cambios guardados", "La información se actualizó correctamente.");
 ...
 async function remove(row) {
  ...
  setBusy(true);
  try { await deleteTestimonial(kind, row.id); refresh(); }
  catch (e) { await showError(e.message); } finally { setBusy(false); }
 }
 function selectFile(event) {
  const selected = event.target.files?.[0];
  if (selected && (!/\.(jpe?g|png|webp)$/i.test(selected.name) || selected.size > 5 * 1024 * 1024)) {
   event.target.value = ""; setFile(null); warning("Imagen inválida", "Usa JPG, PNG o WEBP de hasta 5 MB."); return;
  }
 ...
 return <section className="admin-news__card testimonials-admin">
  ...
  {editor ? <form className="admin-news-form" noValidate onSubmit={submit}><fieldset disabled={busy}>
   ...
   {loading ? <p role="status">Cargando...</p> : <div className="admin-news-table-wrapper"><table className="admin-news-table"><thead><tr><th>{kind === "reviews" ? "Calificación" : "Foto"}</th><th>{kind === "reviews" ? "Nombre y comentario" : "Título y orden"}</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>
    {rows.map(row => <tr key={row.id}><td data-label={kind === "reviews" ? "Calificación" : "Foto"}>{kind === "reviews" ? <Stars value={row.calificacion} /> : <img src={row.imagen} alt="" width="56" height="56" />}</td><td data-label={kind === "reviews" ? "Nombre y comentario" : "Título y orden"}><strong>{kind === "reviews" ? row.nombre : row.titulo}</strong>{kind === "reviews" && <p className="testimonial-admin-comment">{row.comentario}</p>}{kind === "testimonials" && <small>Orden: {row.orden}</small>}</td><td data-label="Estado">{row.activo ? "Visible" : "Oculto"}</td><td data-label="Acciones"><div className="admin-news-actions">
     <button className="admin-news-secondary" disabled={locked} title="Editar" aria-label="Editar" onClick={() => edit(row)}><i className="bi bi-pencil" aria-hidden="true" /></button>
     <button className="admin-news-secondary" disabled={locked} title={row.activo ? "Ocultar" : "Activar"} aria-label={row.activo ? "Ocultar" : "Activar"} onClick={() => toggle(row)}><i className={row.activo ? "bi bi-eye" : "bi bi-eye-slash"} aria-hidden="true" /></button>
     <button className="admin-news-secondary" disabled={locked} title="Eliminar" aria-label="Eliminar" onClick={() => remove(row)}><i className="bi bi-trash" aria-hidden="true" /></button>
    </div></td></tr>)}
