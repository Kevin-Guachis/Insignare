...
const sections = [{ id: "home", label: "Home" }, { id: "footer", label: "Footer" }, { id: "universities", label: "Universidades" }, { id: "siov", label: "SIOV" }, { id: "testimonials", label: "Testimonios" }, { id: "contacto", label: "Contacto" }, { id: "material", label: "Material Filtrado" }];
...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    listAdminNews().then((data) => { if (active) setRows(data); })
      .catch((error) => { if(active){setError("No se pudo cargar la información.");showError(error.message);} })
  ...
  async function reload() {
    ...
    catch(error){setError("No se pudo cargar la información.");showError(error.message); }
  ...
  function saved(row) {
    ...
    setEditor(null);
    showSuccess("Noticia guardada correctamente.");
    setError("");
    notifyNewsChanged();
  }

  async function hide(target) {
    if (busy || !await confirmAction("¿Ocultar noticia?", `“${target.titulo}” dejará de mostrarse en el Home, pero seguirá almacenada.`, "Ocultar")) return;
    setBusy(true);
    setError("");
    try {
      await hideNews(target.id);
      setRows((current) => current.map((row) => row.id === target.id ? { ...row, activo: 0 } : row));
      showSuccess("Noticia oculta. Se conserva en el historial.");
  ...
  async function remove(target) {
    if (busy || !await confirmAction("¿Eliminar noticia?", `La noticia “${target.titulo}” será eliminada definitivamente.\n\nEsta acción no se puede deshacer.`, "Eliminar")) return;
    setBusy(true); setError("");
    try {
      await destroyNews(target.id);
      setRows(current => current.filter(row => row.id !== target.id));

      showSuccess("Noticia eliminada definitivamente.");
      notifyNewsChanged();
    } catch(error){showError(error.message); }
    finally { setBusy(false); }
  }
  ...
  async function restore(row) {
    ...
    try {
      ...
      showSuccess("Noticia restaurada.");
      notifyNewsChanged();
    } catch(error){showError(error.message); }
    finally { setBusy(false); }
  }
      ...
      <div className="admin-news__container">
        ...
        <section id="panel-home" hidden={section !== "home"} className="admin-news__card">
          <div className="admin-news__toolbar">
            ...
            <button className="admin-news-primary" type="button" disabled={!!editor || busy} onClick={() => setEditor({ mode: "create" })}>Nueva noticia</button>
          </div>
                      ...
                      {rows.map((row) => (
                        <tr key={row.id}>
                          ...
                          <td data-label="Acciones"><div className="admin-news-actions">
                            <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={() => setEditor({ news: row })} title="Editar noticia" aria-label="Editar noticia"><i className="bi bi-pencil" aria-hidden="true" /></button>
                            {row.activo === 1 && <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={() => hide(row)} title="Ocultar noticia" aria-label="Ocultar noticia"><i className="bi bi-eye" aria-hidden="true" /></button>}

                            {row.activo === 0 && <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={() => restore(row)} title="Mostrar noticia" aria-label="Mostrar noticia"><i className="bi bi-eye" aria-hidden="true" /></button>}
                            <button type="button" className="admin-news-secondary admin-icon-button" disabled={busy} onClick={() => remove(row)} title="Eliminar noticia" aria-label="Eliminar noticia"><i className="bi bi-trash" aria-hidden="true" /></button>
                          </div></td>
                        </tr>
                      ))}
        ...
        {section==="material"&&<div id="panel-material"><MaterialResourcesAdmin /></div>}
