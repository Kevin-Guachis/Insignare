...
import { siovWarning,siovConfirm,siovSuccess,siovError } from "../components/siov/alerts";
...
export default function Siov(){
 ...
 useEffect(()=>{let active=true;getSiovContent().then(raw=>{const data=prepareContent(raw);if(active){setContent(data);setError("");}}).catch(e=>{if(active){setError(e.message);siovError(e.message);}});return()=>{active=false;};},[retry]);
 function go(next){setScreen(next);window.scrollTo({top:0,behavior:"smooth"});}
 async function start(data){const invalid=studentError(data);if(invalid){await siovWarning("Datos incompletos",invalid.message);document.getElementById("inp-"+invalid.field)?.focus();return;}setStudent(Object.fromEntries(Object.entries(data).map(([k,v])=>[k,v.trim()])));started.current=performance.now();completed.current=false;recordSiovMetric("started");go("test");}
 async function finish(){if(completed.current)return;completed.current=true;const ranked=calculateResults(content.AREAS,answers);setResults(ranked);recordSiovMetric("completed",undefined,Math.min(240,Math.round((performance.now()-started.current)/60000)));recordSiovMetric("area",ranked[0].id);await siovSuccess("Evaluación completada","Tu evaluación ha finalizado y tus resultados están listos.");go("results");}
 async function reset(){if(!await siovConfirm("¿Reiniciar evaluación?","Se perderá el progreso actual de la evaluación.","Sí, reiniciar"))return;setStudent({});setAnswers({});setResults([]);completed.current=false;setContent(null);setRetry(n=>n+1);go("intro");}
